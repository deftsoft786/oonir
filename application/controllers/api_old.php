	...
	class Api extends CI_Controller
	{
		...
		{			
			...
			{		
				...
				if(!empty($user_id))
				{
					$query = $this->db->query('select message,DATE_FORMAT(created_date,"%d-%m-%Y %h:%i %p") as created_date from truck_notification where user_id="'.$user_id.'" and type="customer" order by created_date desc');
					if($query->num_rows()>0)
					{
						$result_data = $query->result_array();
						$msg['return'] = 1;
		...
		{			
			...
			{		
				...
				if(!empty($user_id))
				{
					$query = $this->db->query('select message,DATE_FORMAT(created_date,"%d-%m-%Y %h:%i %p") as created_date from truck_notification where user_id="'.$user_id.'" and type="transporter" order by created_date desc');
					if($query->num_rows()>0)
					{
						$result_data = $query->result_array();
						$msg['return'] = 1;
		...
		**************************************************************************/
		...
		function post_shipment_offer($user_id = '',$truck_id='',$transporter_id='',$pickup_location='',$pickup_city='',$dropup_location='',$dropup_city='',$pickup_date='',$capacity='',$load_type='',$tracking='',$sharing='',$distance='')
		{
			if(!empty($_REQUEST) && isset($_REQUEST))
			{		
				extract($_REQUEST);
				...
				{	
					...
					if($select_truck==2) // ********Truck not selected**********
					{						
						$query = $this->db->query('select * from truck_shipment where pickup_location="'.$pickup_location.'" and dropup_location="'.$dropup_location.'" and pickup_date="'.$pickup_date.'" and capacity="'.$capacity.'" and shipment_type=2 and load_type="'.$load_type.'"');
						if($query->num_rows()>0)
						{
		...
		/******************************* Post Bid **********************************/
		function post_shipment_bid($user_id = '',$pickup_location='',$dropup_location='',$pickup_city='',$dropup_city='',$pickup_date='',$capacity='',$load_type='',$tracking='',$sharing='',$shipment_type='',$distance='')
		{
			if(!empty($_REQUEST) && isset($_REQUEST))
			{		
				extract($_REQUEST);
		...
		function shipment_customer($user_id = '')
		{
			...
			{		
				...
				{					
					...
					$query = $this->db->query('select COALESCE(tr.tkreq_id,"") as tkreq_id,COALESCE(tr.request_status,"") as request_status  ,COALESCE(tr.truck_id,"") as truck_id,COALESCE(tr.bid_value,"") as bid_value,ts.dispatch_status,ts.shipment_id,ts.user_id,ts.offer_requested,ts.pickup_location,ts.dropup_location,ts.pickup_city,ts.dropup_city,DATE_FORMAT(ts.pickup_date,"%d-%m-%Y") as pickup_date,ts.load_type,ts.capacity,ts.tracking,ts.sharing,ts.shipment_type,ts.offered_price,ts.distance,ts.created_date,COALESCE(u.username,"") as transporter_name,COALESCE(u.rating,"") as rating ,COALESCE(u.mobile_number,"") as mobile_number,COALESCE(u.user_id,"") as transporter_id,tl.driver_name,tl.registration_no,ts.platitude,ts.plongitude,ts.dlatitude,ts.dlongitude from truck_shipment ts LEFT JOIN truck_request tr on  ts.shipment_id=tr.shipment_id  and ts.user_id=tr.customer_id LEFT JOIN  user u  on u.user_id=tr.transporter_id LEFT JOIN truck_list tl on tr.truck_id=tl.truck_id where (ts.user_id="'.$user_id.'" and ts.dispatch_status!=5) AND (ts.user_id="'.$user_id.'" and ts.dispatch_status!=6) group by ts.shipment_id order by ts.pickup_date');
			...
			{		
				...
				{					
					...
					if($shipment_type==1) // Bid
					{
						...
						$query = $this->db->query('select tr.tkreq_id,tr.request_status ,tr.truck_id,tr.bid_value,ts.shipment_id,ts.pickup_location,ts.dropup_location,ts.dropup_city,ts.pickup_city,ts.platitude,ts.plongitude,ts.dlatitude,ts.dlongitude,DATE_FORMAT(ts.pickup_date,"%d-%m-%Y") as pickup_date,ts.load_type,ts.capacity,ts.tracking,ts.sharing,ts.shipment_type,ts.offered_price,ts.distance,u.user_id as transporter_id,u.username as transporter_name,u.rating,u.mobile_number from truck_request tr inner join truck_shipment ts INNER JOIN user u on ts.shipment_id=tr.shipment_id and u.user_id = tr.transporter_id where tr.customer_id="'.$user_id.'" and tr.shipment_id="'.$shipment_id.'" and tr.request_status="1" and tr.shipment_type="1"  order by tr.bid_value asc');						
						if($query->num_rows()>0)
						{
							$result_data = $query->result_array();					
							$msg['return'] 	=  1;
					...
					{
						$query = $this->db->query('select tr.tkreq_id,tr.request_status,tr.truck_id,tr.bid_value,ts.shipment_id,ts.pickup_location,ts.dropup_location,ts.dropup_city,ts.pickup_city,ts.platitude,ts.plongitude,ts.dlatitude,ts.dlongitude,DATE_FORMAT(ts.pickup_date,"%d-%m-%Y") as pickup_date,ts.load_type,ts.capacity,ts.tracking,ts.sharing,ts.shipment_type,ts.offered_price,ts.distance,tl.driver_name,tl.contact_number,tl.registration_no ,u.username as transporter_name,u.user_id as transporter_id,u.rating,u.mobile_number  from truck_request tr inner join truck_shipment ts INNER JOIN truck_list tl INNER JOIN user u on ts.shipment_id=tr.shipment_id and tl.truck_id=tr.truck_id and u.user_id = tr.transporter_id where (tr.customer_id="'.$user_id.'" and tr.shipment_id="'.$shipment_id.'" and tr.shipment_type=2 and tr.request_status=1) 
				...
				{					
					...
					{						
						...
						if(!empty($source))
						{
							...
							$query = $this->db->query('select ts.user_id as customer_id,ts.shipment_id,ts.pickup_location,ts.dropup_location,DATE_FORMAT(ts.pickup_date,"%d-%m-%Y") as pickup_date,ts.pickup_city,ts.dropup_city,ts.platitude,ts.plongitude,ts.dropup_city,ts.dlatitude,ts.dlongitude,ts.load_type,ts.capacity,ts.tracking,ts.sharing,ts.shipment_type,ts.offered_price,ts.distance,DATE_FORMAT(ts.created_date,"%d-%m-%Y") as created_date
													   ,SQRT(POW(69.1*(ts.platitude- '.$platitude.'),2)+ POW(69.1*('.$plongitude.'-ts.plongitude)* COS(ts.platitude/57.3),2)) AS distances from  truck_shipment ts where  ts.pickup_location="'.$source.'" and  (ts.dispatch_status=1 OR ts.dispatch_status=2) HAVING distances < '.$distance.' order by ts.created_date desc');					
						...
						else if(!empty($destination))
						{
							...
							$query = $this->db->query('select ts.user_id as customer_id,ts.shipment_id,ts.pickup_location,ts.dropup_location,DATE_FORMAT(ts.pickup_date,"%d-%m-%Y") as pickup_date,ts.pickup_city,ts.dropup_city,ts.platitude,ts.plongitude,ts.dropup_city,ts.dlatitude,ts.dlongitude,ts.load_type,ts.capacity,ts.tracking,ts.sharing,ts.shipment_type,ts.offered_price,ts.distance ,DATE_FORMAT(ts.created_date,"%d-%m-%Y") as created_date
									 ,SQRT(POW(69.1*(ts.dlatitude- '.$dlatitude.'),2)+ POW(69.1*('.$dlongitude.'-ts.dlongitude)* COS(ts.dlatitude/57.3),2)) AS distances  from  truck_shipment ts where  ts.dropup_location="'.$destination.'" and (ts.dispatch_status=1 OR ts.dispatch_status=2) HAVING distances < '.$distance.'   order by ts.created_date desc');
					...
					{						
						...
						$query = $this->db->query('select ts.user_id as customer_id,ts.shipment_id,ts.pickup_location,ts.dropup_location,DATE_FORMAT(ts.pickup_date,"%d-%m-%Y") as pickup_date,ts.pickup_city,ts.dropup_city,ts.platitude,ts.plongitude,ts.dropup_city,ts.dlatitude,ts.dlongitude,ts.load_type,ts.capacity,ts.tracking,ts.sharing,ts.shipment_type,
						ts.offered_price,ts.distance,ts.dispatch_status,DATE_FORMAT(ts.created_date,"%d-%m-%Y") as created_date,tl.truck_id,tl.driver_name,tl.contact_number,tl.registration_no ,u.username as transporter_name 
						from  truck_shipment ts INNER JOIN truck_list tl INNER JOIN user u on  u.user_id = tl.user_id where 
						(tl.user_id="'.$user_id.'" and (ts.dispatch_status=1 OR ts.dispatch_status=2) and tl.pickup_location=ts.pickup_location and tl.dropup_location=ts.dropup_location) 
						OR (tl.user_id="'.$user_id.'" and (ts.dispatch_status=1 OR ts.dispatch_status=2)  and tl.pickup_location=ts.dropup_location and tl.dropup_location=ts.pickup_location) 
						OR (tl.user_id="'.$user_id.'" and (ts.dispatch_status=1 OR ts.dispatch_status=2)  and tl.pickup_location=ts.pickup_location and tl.inbetween_location LIKE CONCAT("%",ts.dropup_location,"%")) 
						OR (tl.user_id="'.$user_id.'" and(ts.dispatch_status=1 OR ts.dispatch_status=2) and tl.inbetween_location LIKE CONCAT("%",ts.pickup_location,"%") and tl.dropup_location=ts.dropup_location)
						OR (tl.user_id="'.$user_id.'" and (ts.dispatch_status=1 OR ts.dispatch_status=2)  and tl.inbetween_location LIKE CONCAT("%",ts.pickup_location,"%") and tl.inbetween_location LIKE CONCAT("%",ts.dropup_location,"%"))						
		...
		function accept_offer_by_transporter($shipment_id='',$tkreq_id='',$customer_id='')
		{
			...
			{		
				...
				{					
					$query = $this->db->query('update truck_request set request_status="3" where tkreq_id="'.$tkreq_id.'" and shipment_id="'.$shipment_id.'"');
					if($query)
					{
						$query1 = $this->db->query('update truck_shipment set dispatch_status="3" where shipment_id="'.$shipment_id.'"');						
						$query2 = $this->db->query('delete from truck_request where shipment_id="'.$shipment_id.'" and request_status=0');					
						$query3 = $this->db->query('update truck_shipment set offer_requested="0" where shipment_id="'.$shipment_id.'" and offer_requested=1');
						...
						if(!empty($device_token))
						{
							...
							$message = 'Your shipment offer has been accepted by transporter';							
							$data = array(
										'sound'		=>1,
										'message'	=>$message,
		...
		function reject_offer_by_transporter($shipment_id='',$tkreq_id='',$customer_id='')
		{
			...
			{		
				...
				if(!empty($tkreq_id)&& !empty($shipment_id)&&!empty($customer_id))
				{					
					$query = $this->db->query('delete from truck_request where tkreq_id="'.$tkreq_id.'" and shipment_id="'.$shipment_id.'"');
					if($query)
					{					
						...
						if(!empty($device_token))
						{
							...
							$device_token = $device_token[0]['device_token']; 							
							$msgsender_id['user_id'] 	 = $customer_id;	
							$message = 'Your shipment offer has been rejected by transporter';							
							$data = array(
										'sound'		=>1,
										'message'	=>$message,
		...
		function accept_requestby_transporter($customer_id='',$shipment_id='',$transporter_id='',$shipment_type='',$bid_value='',$truck_id='')
		{
			...
			{		
				...
				{					
					...
					(customer_id="'.$customer_id.'" and shipment_id="'.$shipment_id.'" and transporter_id="'.$transporter_id.'" and request_status=1) 
					OR (customer_id="'.$customer_id.'" and shipment_id="'.$shipment_id.'" and transporter_id="'.$transporter_id.'" and request_status=3)
					OR (customer_id="'.$customer_id.'" and shipment_id="'.$shipment_id.'" and transporter_id="'.$transporter_id.'" and request_status=4)');
					if($query->num_rows()>0)
					{
						$msg['return'] 	=  0;
					...
					if($query1->num_rows()==0)
					{
						...
						{									
							...
							$msgsender_id['user_id'] 	 = $customer_id;	
							$message = 'Your shipment has been accepted by transporter';							
							$data = array(
										'sound'		=>1,
										'message'	=>$message,
		...
		function book_shipment($tkreq_id='',$shipment_id='',$truck_id='')
		{
			...
			{		
				...
				{					
					$query = $this->db->query('select tkreq_id,transporter_id from truck_request where tkreq_id="'.$tkreq_id.'"');
					if($query->num_rows()>0)
					{	
						$result_data = $query->result_array();
						...
						{	
							...
							{					
								...
								$message = 'Your shipment has been booked by customer';							
								$data = array(
											'sound'		=>1,
											'message'	=>$message,
		...
		/******************************* shipment booking cancel by customer/transporter **********************************/
				...
				{					
					...
					{									
						...
						if($query_rs)
						{
							...
							$query = $this->db->query('select transporter_id from truck_request where tkreq_id="'.$tkreq_id.'"');
							if($query->num_rows()>0)
							{	
								$result_data = $query->result_array();
								$transporter_id = $result_data[0]['transporter_id'];
								...
								$msgsender_id['user_id'] = $transporter_id;	
								$message = 'Your shipment booking has been cancelled';							
								$data = array(
											'sound'		=>1,
											'message'	=>$message,
		...
		function cancel_request($tkreq_id='',$shipment_id='',$truck_id='',$transporter_id='',$type='')
		{
			...
			{		
				...
				{					
					if($type=="2") // customer
					{									
						$query_rs = $this->db->query('delete from truck_request where tkreq_id="'.$tkreq_id.'" and shipment_id="'.$shipment_id.'" and truck_id="'.$truck_id.'" and request_status=0');
						if($query_rs)
						{						
							/************* Send notificaion to transporter ************/
							...
							$message1 = 'Your shipment request cancelled';							
							$data1 = array(
										'sound'		=>1,
										'message'	=>$message1,
					...
					if($type=="1") // transporter					
					{
						...
						{	
							...
							// $query_rs = $this->db->query('update truck_shipment set request_status="5" where tkreq_id="'.$tkreq_id.'" and shipment_id="'.$shipment_id.'" and truck_id="'.$truck_id.'" and request_status=3');
		...
		/******************************* shipment cancel by transporter **********************************/
		function booking_cancel_transporter($tkreq_id='',$shipment_id='',$truck_id='')
		{
			if(!empty($_REQUEST) && isset($_REQUEST))
			{		
				extract($_REQUEST);
				...
				{					
					$query = $this->db->query('select tkreq_id from truck_request where tkreq_id="'.$tkreq_id.'"');
					if($query->num_rows()>0)
					{				
						$query_rs = $this->db->query('update truck_request set request_status="5" where tkreq_id="'.$tkreq_id.'" and shipment_id="'.$shipment_id.'" and truck_id="'.$truck_id.'" and request_status=3');
						if($query_rs)
						{	
							// $query = $this->db->query('update truck_shipment set dispatch_status="5" where shipment_id="'.$shipment_id.'" and dispatch_status=3');
		...
		function myshipment_transport($user_id = '')
		{
			...
			{		
				...
				if(!empty($user_id))
				{					
					$query = $this->db->query('select tr.tkreq_id,tr.shipment_id,tr.customer_id,tr.transporter_id,tr.truck_id,tr.bid_value,tr.request_status,ts.shipment_id,ts.pickup_location,ts.dropup_location,DATE_FORMAT(ts.pickup_date,"%d-%m-%Y") as pickup_date,ts.load_type,ts.capacity,ts.tracking,ts.sharing,ts.shipment_type,ts.offered_price,ts.platitude,ts.plongitude,ts.pickup_city,ts.dropup_city,ts.dlatitude,ts.dlongitude,ts.offered_price,ts.distance,ts.dispatch_status,tl.driver_name,tl.truck_code,tl.contact_number,tl.registration_no ,u.username as transporter_name from truck_request tr inner join truck_shipment ts INNER JOIN truck_list tl INNER JOIN user u on ts.shipment_id=tr.shipment_id and tl.truck_id=tr.truck_id and u.user_id = tr.transporter_id where (tr.transporter_id="'.$user_id.'" and tr.request_status!=2) and (tr.transporter_id="'.$user_id.'" and tr.request_status!=5) and (tr.transporter_id="'.$user_id.'" and tr.request_status!=6) order by ts.pickup_date');
		...
		function myshipment_driver($truck_id = '')
		{
			...
			{		
				...
				{					
					...
					$query = $this->db->query('select tr.tkreq_id,tr.shipment_id,tr.customer_id,tr.transporter_id,tr.truck_id,tr.request_status,tr.bid_value,ts.shipment_id,ts.pickup_location,ts.dropup_location,ts.pickup_city,ts.dropup_city,ts.platitude,ts.plongitude,ts.dlatitude,ts.dlongitude,DATE_FORMAT(ts.pickup_date,"%d-%m-%Y") as pickup_date,ts.load_type,ts.capacity,ts.tracking,ts.sharing,ts.shipment_type,ts.offered_price,tl.driver_name,tl.registration_no ,u.username as transporter_name,u.mobile_number as contact_number from truck_request tr inner join truck_shipment ts INNER JOIN truck_list tl INNER JOIN user u on ts.shipment_id=tr.shipment_id and tl.truck_id=tr.truck_id and u.user_id = tr.transporter_id where (tr.truck_id="'.$truck_id.'" and tr.request_status=3) OR (tr.truck_id="'.$truck_id.'" and tr.request_status=4)');
					...
					{	
						...
						{	
							...
							if($query)
							{
								...
								$msgsender_id1['user_id'] 	 = $transporter_id;	
								$message1 = 'Your shipment has been dispatched';							
								$data1 = array(
											'sound'		=>1,
											'message'	=>$message1,
											'notifykey'	=>'shipment_dispatched',
					...
					{	
						...
						{	
							...
							if($query)
							{
								...
								$msgsender_id1['user_id'] 	 = $transporter_id;	
								$message1 = 'Your shipment has been delivered';							
								$data1 = array(
											'sound'		=>1,
											'message'	=>$message1,
											'notifykey'	=>'shipment_delivered',
		...
		function history_transport($user_id = '')
		{
			...
			{		
				...
				if(!empty($user_id))
				{					
					$query = $this->db->query('select tr.tkreq_id,tr.shipment_id,tr.customer_id,tr.transporter_id,tr.truck_id,tr.bid_value,tr.request_status,ts.shipment_id,ts.pickup_location,ts.pickup_city,ts.platitude,ts.plongitude,ts.dropup_location,ts.dropup_city,ts.dlatitude,ts.dlongitude,DATE_FORMAT(ts.pickup_date,"%d-%m-%Y") as pickup_date,ts.load_type,ts.capacity,ts.tracking,ts.sharing,ts.shipment_type,ts.offered_price,tl.driver_name,tl.contact_number,tl.registration_no ,tl.truck_code,u.username,u.mobile_number,u.rating  from truck_request tr inner join truck_shipment ts INNER JOIN truck_list tl INNER JOIN user u on ts.shipment_id=tr.shipment_id and tl.truck_id=tr.truck_id and u.user_id = tr.customer_id where (tr.transporter_id="'.$user_id.'" and tr.request_status=5) OR (tr.transporter_id="'.$user_id.'" and tr.request_status=6)');
					if($query->num_rows()>0)
					{
						$result_data = $query->result_array();
		...
		function history_customer($user_id = '')
		{
			...
			{		
				...
				if(!empty($user_id))
				{					
					$query = $this->db->query('select tr.tkreq_id,tr.shipment_id,tr.customer_id,tr.transporter_id,tr.truck_id,tr.bid_value,tr.request_status,ts.shipment_id,ts.pickup_location,ts.pickup_city,ts.platitude,ts.plongitude,ts.dropup_location,ts.dropup_city,ts.dlatitude,ts.dlongitude,DATE_FORMAT(ts.pickup_date,"%d-%m-%Y") as pickup_date,ts.load_type,ts.capacity,ts.tracking,ts.sharing,ts.shipment_type,ts.offered_price,tl.driver_name,tl.contact_number,tl.registration_no ,u.username as transporter_name,u.mobile_number,u.rating from truck_request tr inner join truck_shipment ts INNER JOIN truck_list tl INNER JOIN user u on ts.shipment_id=tr.shipment_id and tl.truck_id=tr.truck_id and u.user_id = tr.transporter_id where (tr.customer_id="'.$user_id.'" and tr.request_status=5) OR (tr.customer_id="'.$user_id.'" and tr.request_status=6)');
					if($query->num_rows()>0)
					{
						$result_data = $query->result_array();
		...
		function history_driver($user_id = '')
		{
			...
			{		
				...
				if(!empty($user_id))
				{					
					$query = $this->db->query('select tr.tkreq_id,tr.shipment_id,tr.customer_id,tr.transporter_id,tr.truck_id,tr.bid_value,tr.request_status,ts.shipment_id,ts.pickup_location,ts.dropup_location,ts.pickup_city,ts.dropup_city,DATE_FORMAT(ts.pickup_date,"%d-%m-%Y") as pickup_date,ts.platitude,ts.plongitude,ts.dlongitude,ts.dlatitude,ts.load_type,ts.capacity,ts.tracking,ts.sharing,ts.shipment_type,ts.offered_price,tl.driver_name,tl.registration_no ,u.username as transporter_name,u.mobile_number as contact_number from truck_request tr inner join truck_shipment ts INNER JOIN truck_list tl INNER JOIN user u on ts.shipment_id=tr.shipment_id and tl.truck_id=tr.truck_id and u.user_id = tr.transporter_id where (tr.truck_id="'.$user_id.'" and tr.request_status=5) OR  (tr.truck_id="'.$user_id.'" and tr.request_status=6)');
					if($query->num_rows()>0)
					{
						$result_data = $query->result_array();
						for($i=0;$i<count($result_data);$i++)
		...
		function get_message($send_by='',$send_to='')
		{
			...
			{		
				...
				{
					$query = $this->db->query("select * from truck_chat where send_to='".$send_to."' and send_by='".$send_by."' or send_to='".$send_by."' and send_by='".$send_to."' order by  created_date  asc");
		...
		function rating($customer_id = '',$transporter_id='',$rating='')
		{
			if(!empty($_REQUEST) && isset($_REQUEST))
			{		
				extract($_REQUEST);
				...
				if(!empty($customer_id) && !empty($transporter_id)&& !empty($rating))
				{
					...
					$msg['return'] = 0;
					$msg['result'] = "please provide parameter: customer_id,transporter_id,rating";
				}
			}else{
				$msg['return'] = 0;
				$msg['result'] = "please provide parameter: customer_id,transporter_id,rating";
				...
				if(!empty($email))
				{
					...
					else
					{
						...
						if($query->num_rows()>0)
						{
							...
							$this->email->message(" Hello <b>$name<b> \n Your Password is $password");
		...
		{			
			...
			$pdf->SetFont('Arial','B');
			$pdf->Cell(180,5,"Retail Invoice ",1,1,'C');
			...
			$pdf->SetTextColor(0,0,0);
			$pdf->SetFont('Arial','B');
			$pdf->Cell(180,10," ",1,0,'L');
			
			
			$to 	= "testing.testing139@gmail.com";
			$from 	= "TruckDispatch";
			// $from 	= "riya.sen485@gmail.com";
			...
			echo $new_dir = dirname(dirname(__FILE__))."/upload/car_image/".$filename;
